...
class Events(object):
    ...
    def location_filter(self, location):
        ...
        new_event_list = [
            event for event in self.event_list if event['location'] == location]
        return new_event_list

    def category_filter(self, category):
        """A method to find events of a given category """
        new_event_list = [
            event for event in self.event_list if event['category'] == category]
        return new_event_list

    def createdby_filter(self, username):
        """Filter and return the events created by a particular user"""
        new_event_list = [
            event for event in self.event_list if event['createdby'] == username]
        return new_event_list
